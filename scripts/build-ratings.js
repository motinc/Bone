...
function train(matches){
  ...
  // the four best-of-3 outcomes from the FAVOURITE's perspective:
  //   fav 2-0, fav 2-1, fav 1-2 (loses), fav 0-2 (loses).
  // Measured empirically as we train (pre-update ratings = a genuine forecast).
  // straightSets (P fav 2-0) is derived from this for set-handicap pricing.
  const ssBins = {}; // bin(0..9) -> { n, fav20, fav21, fav12, fav02 }
  ...
  for(const m of matches){
    ...
    if(Math.min(w.matches,l.matches) >= 20){
      ...
      if(sc){
        ...
        ssBins[bin].n++;
        // From the favourite's perspective. sc.ws/sc.ls are winner/loser sets.
        if(wIsFav){
          // favourite won: 2-0 if loser took 0 sets, else 2-1
          if(sc.ls===0) ssBins[bin].fav20++; else ssBins[bin].fav21++;
        } else {
          // favourite lost: they (the loser of the match) took sc.ls sets.
          // fav 1-2 if favourite took 1 set, fav 0-2 if they took none.
  ...
  // straightSets[bin] = P(fav 2-0)  (kept for set-handicap pricing, unchanged shape)
  // scorelines[bin]   = { p20, p21, p12, p02 }  (full distribution, sums to ~1)
  const straightSets = {};
  const scorelines = {};
  for(let b=0;b<10;b++){
    const x=ssBins[b];
  ...
  return { R, straightSets, scorelines };
}
...
(async()=>{
  ...
  const { R, straightSets, scorelines } = train(matches);
  ...
  const out={
    ...
    scorelines,     // full scoreline distribution by favProb bin
