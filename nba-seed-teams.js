...
// Days to walk back. Default = full backfill. The scheduled workflow passes a
// small number (e.g. `node nba-seed-teams.js --days 5`) to only append recent
// finished games cheaply, rather than re-walking the whole season every run.
const argDays = (() => {
  const i = process.argv.indexOf('--days');
  return i !== -1 && process.argv[i+1] ? parseInt(process.argv[i+1], 10) : null;
})();
const DAYS_BACK = argDays && argDays > 0 ? argDays : 220;   // full backfill default
